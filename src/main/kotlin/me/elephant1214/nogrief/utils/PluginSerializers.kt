...
object ClaimSerializer : KSerializer<Claim> {
    ...
    private const val WORLD_INDEX = 2
    private const val CHUNKS_INDEX = 3
    private const val PERMS_INDEX = 4
    private const val MODIFIED_INDEX = 5
    ...
    override val descriptor: SerialDescriptor = buildClassSerialDescriptor("claim") {
        ...
        element<Map<@Serializable(UUIDSerializer::class) UUID, @Serializable(ClaimPermsSerializer::class) EnumSet<ClaimPermission>>>(
            "permissions"
        )
    ...
    override fun serialize(encoder: Encoder, value: Claim) {
        ...
        composite.encodeSerializableElement(
            descriptor,
            CHUNKS_INDEX,
            SetSerializer(Long.serializer()),
            value.getChunksForSerial()
        )
        composite.encodeSerializableElement(
            descriptor, PERMS_INDEX, MapSerializer(UUIDSerializer, ClaimPermsSerializer), value.getPermsForSerial()
        )
    ...
    override fun deserialize(decoder: Decoder): Claim {
        ...
        loop@ while (true) {
            when (val index = composite.decodeElementIndex(descriptor)) {
                ...
                CHUNKS_INDEX -> chunks = composite.decodeSerializableElement(descriptor, index, SetSerializer(Long.serializer()))
                PERMS_INDEX -> permissions = composite.decodeSerializableElement(
                    descriptor, index, MapSerializer(UUIDSerializer, ClaimPermsSerializer)
                )
        ...
        return Claim(claimId, owner, world, ClaimChunk.fromLongSet(world, chunks), permissions.toMutableMap(), modified)
    }
